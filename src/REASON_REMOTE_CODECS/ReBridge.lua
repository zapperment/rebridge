...
function remote_deliver_midi(max_bytes, port)
  ...
  for _,item in pairs(items) do
    ...
    if port==2 and item.dispatchToLive then
        table.insert(events,remote.make_midi("f0 7f 7f 7f 7f 00 xx yy f7", {x=item.controller, y=remote.get_item_value(item.index)}))
        item.dispatchToLive=false
    end
  end
  return events
...
--    response="f0 7e 00 06 02 00 20 29 13 01 00 00 ?? ?? ?? ?? f7"
--  }
--end
