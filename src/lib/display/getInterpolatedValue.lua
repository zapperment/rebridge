...
return function(deviceType, paramName, value)
  ...
  if (ivForDevice == nil) then
    debug.log("[lib.display.getInterpolatedValue] deviceType=" ..
  ...
  if mode == const.interpolation.bipolar then
    debug.log("[lib.display.getInterpolatedValue] => scaling in bipolar mode")
    scaled = calc.scaleBipolar(value, min, max)
  elseif mode == const.interpolation.linear then
    debug.log("[lib.display.getInterpolatedValue] => scaling in linear mode")
    scaled = calc.scale(value, min, max)
  elseif mode == const.interpolation.reciprocal then
    debug.log("[lib.display.getInterpolatedValue] => scaling in reciprocal mode")
    scaled = calc.scaleReciprocal(value)
  else
    debug.log("[lib.display.getInterpolatedValue] => unknown mode, no scaling")
  ...
  debug.log("[lib.display.getInterpolatedValue] => return value: " .. retVal)
  return retVal
end
